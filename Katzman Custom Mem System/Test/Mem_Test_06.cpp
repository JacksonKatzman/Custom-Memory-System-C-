...
TEST(Allocate_3_bottom_up_free, MemoryShakeOut)
{
	...
	// ---- Verify A: -----------------------------------------------------------
		...
		// UsedHdr / FreeHdr
		CHECK_EQUAL(PTR_FIX(h->pFreeHead), 0x670);
		CHECK_EQUAL(PTR_FIX(h->pNextFit), 0x670);
	...
	// ---- Verify B: -----------------------------------------------------------
		...
		// UsedHdr / FreeHdr
		CHECK_EQUAL(PTR_FIX(h->pFreeHead), 0x40 + 0x10 + 0x200 + 0x10 + 0x200);
		CHECK_EQUAL(PTR_FIX(h->pNextFit), 0x40 + 0x10 + 0x200 + 0x10 + 0x200);
		CHECK_EQUAL(PTR_FIX(h->pUsedHead), 0x250);
	...
	// ---- Heap Stats ------------------------------------------------------
		...
		CHECK_EQUAL(h->mStats.currNumFreeBlocks, 1);
		CHECK_EQUAL(h->mStats.currFreeMem, 0xc180 + 0x200 + 0x10);
	...
	// ----  Block walk ------------------------------------------------------
		...
		// ---- UsedHdr free 3 -------------------------------------------
			...
			hdrStart = (Type::U32)free;
			hdrEnd = (Type::U32)free + sizeof(UsedHdr);
		...
		// ---- used BLOCK ----------------------------------------------------
			...
			CHECK_EQUAL(PTR_FIX(blkStart), PTR_FIX(hdrEnd));
			CHECK_EQUAL(PTR_FIX(blkEnd), 0xc800);
			CHECK_EQUAL(free->mBlockSize, 0xc390);
	...
	// ---- Verify C: -----------------------------------------------------------
		...
		CHECK_EQUAL(PTR_FIX(h->pNextFit), 0x40 + 0x10 + 0x200);
